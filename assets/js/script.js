...
if (form) { // Memastikan form elemen ada
  form.addEventListener("submit", function(event) {
      ...
      if (response.ok) {
        ...
        response.json().then(data => {
          if (Object.hasOwnProperty.call(data, 'errors')) {
            alert("Oops! Ada masalah saat mengirim pesan Anda: " + data["errors"].map(error => error["message"]).join(", "));
          } else {
            alert("Oops! Ada masalah saat mengirim pesan Anda.");
          }
        }).catch(() => {
            alert("Oops! Server error saat mengirim pesan Anda.");
        });
    ...
    .catch(error => {
      alert("Oops! Ada masalah koneksi: " + error.message);
  ...
  navigationLinks[i].addEventListener("click", function () {
    ...
    for (let j = 0; j < pages.length; j++) { // Ganti i dengan j untuk loop pages
      if (this.dataset.pageBtn === pages[j].dataset.page) { // Gunakan data-page-btn
        ...
        // navigationLinks[j].classList.remove("active"); // Hapus active dari link lain
      }
    }
